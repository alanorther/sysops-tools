...
def get_access_key():
    for x in aws_env:
        for r in aws_region:
            security_groups = subprocess.call("aws ec2 describe-instances --region " + r + " --profile "
                                                      + x.translate(None, '[]') + " | grep " + user_key.strip(), shell=True)
            if security_groups == 0:
                sg_out = subprocess.check_output("aws ec2 describe-instances --region " + r + " --profile "
                                                 + x.translate(None, '[]') + " --instance-ids " + user_key.strip(), shell=True)
                print sg_out
                print "Instance in Region: " + r
                quit()

def print_help():
    print "usage: " + argv[0] + " [options....] "
    print "options:"
    print "-i, --instance-id       The instance id you are looking for"
    quit()
...
print "Searching for instance id " + user_key.strip()
