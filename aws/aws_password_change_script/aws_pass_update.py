...
def which_pass_change():
    global aws_pass_env, user_env, pass_num
    global create_pass, new_pass
    i = 0
    o = 0
    ...
    for u, p in zip(user_env, aws_pass_env):
        i += 1
        print("[" + str(i) + "] " + "UserName: " + u.translate(None, '"\n\r')  + "\t\tEnvironment: " + p.translate(None, '[]'))

    while True:
        n = int(input("Which password would you like to change: [1 - " + str(i) + "]"))
        if 1 <= n <= i:
            pass_num = n
            print "Okay, changing password: " + str(pass_num)
            #create_pass = query_yes_no("Would you like me to autogenerate a password for you based off your password policy?", "yes")
            create_pass = True
            if create_pass == False:
                o = 0
                for u, p in zip(user_env, aws_pass_env):
                    ...
                    if o == pass_num:
                        ...
                        print "Require lowercase: " + req_lower_case + "Require uppercase: " + req_upper_case + "Minimum length: " + min_len + "Require number: " + req_num + "Require special character: " + req_sym
                new_pass = getpass.getpass()
                new_pass2 = getpass.getpass("Confirm password: ")
                if new_pass in new_pass2:
                    o = 0
                    for u, p in zip(user_env, aws_pass_env):
                        o += 1
                        if o == i:
...
if change_all_passwords == True:
    ...
    print "Currently only option is to change all passwords, will fix soon, Goodbye!"
    exit(-1)
    # which_pass_change()
